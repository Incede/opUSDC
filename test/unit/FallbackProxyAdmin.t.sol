...
abstract contract Base is Helpers {
  ...
  function setUp() public virtual {
    vm.prank(_owner);
    admin = new FallbackProxyAdmin(_eurc);
  }
}
...
contract FallbackProxyAdmin_Unit_ChangeAdmin is Base {
  ...
  /**
   * @notice Check that the contract calls the `changeAdmin` function of the EURC contract
   */
  function test_changeAdmin(address _newOwner) public {
    vm.assume(_newOwner != _owner);

    _mockAndExpect(_eurc, abi.encodeWithSignature('changeAdmin(address)', _newOwner), abi.encode());
    vm.prank(_owner);
    admin.changeAdmin(_newOwner);
  }
}
...
contract FallbackProxyAdmin_Unit_UpgradeTo is Base {
  ...
  /**
   * @notice Check that the contract calls the `upgradeTo` function of the EURC contract
   */
  function test_upgradeTo(address _newImplementation) public {
    _mockAndExpect(_eurc, abi.encodeWithSignature('upgradeTo(address)', _newImplementation), abi.encode());
    vm.prank(_owner);
    admin.upgradeTo(_newImplementation);
  }
}
...
contract FallbackProxyAdmin_Unit_UpgradeToAndCall is Base {
  ...
  /**
   * @notice Check that the contract calls the `upgradeToAndCall` function of the EURC contract
   */
  function test_upgradeToAndCall(address _newImplementation, bytes memory _data) public {
    _mockAndExpect(
      _eurc, abi.encodeWithSignature('upgradeToAndCall(address,bytes)', _newImplementation, _data), abi.encode()
    );
    vm.prank(_owner);
    admin.upgradeToAndCall(_newImplementation, _data);
  }
