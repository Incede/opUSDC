...
import {IntegrationLisk} from './IntegrationLisk.sol';
import {IOpEURCBridgeAdapter} from 'interfaces/IOpEURCBridgeAdapter.sol';

contract Integration_BridgingLisk is IntegrationLisk {
  ...
  function test_bridgeFromL1() public {
    ...
    vm.prank(SEPOLIA_EURC.masterMinter());
    SEPOLIA_EURC.mint(_user, _amount);

    vm.startPrank(_user);
    SEPOLIA_EURC.approve(address(l1Adapter), _amount);
    l1Adapter.sendMessage(_user, _amount, _MIN_GAS_LIMIT);
    vm.stopPrank();

    assertEq(SEPOLIA_EURC.balanceOf(_user), 0);
    assertEq(SEPOLIA_EURC.balanceOf(address(l1Adapter)), _amount);

    vm.selectFork(lisk);
    uint256 _userBalanceBefore = bridgedEURC.balanceOf(_user);

    _relayL1ToL2Message(
      LISK_ALIASED_L1_MESSENGER,
      address(l1Adapter),
      address(l2Adapter),
      _ZERO_VALUE,
      1_000_000,
  ...
  function test_bridgeFromL1WithSig() public {
    ...
    SEPOLIA_EURC.mint(_signerAd, _amount);
    ...
    assertEq(SEPOLIA_EURC.balanceOf(_signerAd), 0);
    assertEq(SEPOLIA_EURC.balanceOf(_user), _amount);
    assertEq(SEPOLIA_EURC.balanceOf(address(l1Adapter)), _amount);
  ...
  function test_recoverBlacklistedFundsAfterMigration() public {
    ...
    // Mint sepolia EURC to the spender
    vm.prank(SEPOLIA_EURC.masterMinter());
    SEPOLIA_EURC.mint(_spender, _amount);
    ...
    assertEq(SEPOLIA_EURC.balanceOf(_spender), 0);
    ...
    vm.selectFork(sepolia);
    assertEq(SEPOLIA_EURC.isBlacklisted(_spender), false);
    _relayL2ToL1Message(
      address(l2Adapter),
      address(l1Adapter),
      _ZERO_VALUE,
    ...
    assertEq(SEPOLIA_EURC.balanceOf(_spender), _amount);
  }
}

contract Integration_Migration is IntegrationLisk {
  ...
  function setUp() public override {
    ...
    vm.prank(SEPOLIA_EURC.masterMinter());
    SEPOLIA_EURC.configureMinter(address(l1Adapter), 0);
  }
  ...
  function test_migrationToNativeEURC() public {
    _mintSupplyOnL2(lisk, LISK_ALIASED_L1_MESSENGER, _amount);
    ...
    assertEq(l1Adapter.EURC(), address(SEPOLIA_EURC));
    assertEq(uint256(l1Adapter.messengerStatus()), uint256(IOpEURCBridgeAdapter.Status.Deprecated));

    vm.prank(_circle);
    l1Adapter.burnLockedEURC();

    assertEq(SEPOLIA_EURC.balanceOf(address(l1Adapter)), 0);
...
contract Integration_Integration_PermissionedFlows is IntegrationLisk {
  /**
  ...
  function test_userCanWithdrawBlacklistedFunds() public {
    ...
    vm.prank(SEPOLIA_EURC.blacklister());
    SEPOLIA_EURC.blacklist(_user);
    _relayL2ToL1Message(
      address(l2Adapter),
      address(l1Adapter),
      _ZERO_VALUE,
    ...
    vm.prank(SEPOLIA_EURC.blacklister());
    SEPOLIA_EURC.unBlacklist(_user);
