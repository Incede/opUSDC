...
contract Integration_Bridging is IntegrationBase {
  ...
  function test_bridgeFromL2() public {
    ...
    assertEq(bridgedEURC.balanceOf(_user), _amount);
    assertEq(bridgedEURC.balanceOf(address(l2Adapter)), 0);
    ...
    uint256 _userBalanceBefore = MAINNET_EURC.balanceOf(_user);
    _relayL2ToL1Message(
      address(l2Adapter),
      address(l1Adapter),
      _ZERO_VALUE,
    ...
    assertEq(MAINNET_EURC.balanceOf(_user), _userBalanceBefore + _amount);
  ...
  function test_bridgeFromL2DifferentTarget() public {
    ...
    // Mint to increment total supply of bridgedEURC and balance of _user
    vm.prank(address(l2Adapter));
    bridgedEURC.mint(_user, _amount);
    ...
    assertEq(MAINNET_EURC.balanceOf(_l1Target), _userBalanceBefore + _amount);
    assertEq(MAINNET_EURC.balanceOf(address(l1Adapter)), 0);
  }
  ...
  function test_bridgeFromL2WithSig() public {
    ...
    // Mint to increment total supply of bridgedEURC and balance of _user
    vm.prank(address(l2Adapter));
    bridgedEURC.mint(_signerAd, _amount);
    ...
    assertEq(bridgedEURC.balanceOf(_signerAd), 0);
    assertEq(bridgedEURC.balanceOf(_user), _amount);
    assertEq(bridgedEURC.balanceOf(address(l2Adapter)), 0);
    ...
    assertEq(MAINNET_EURC.balanceOf(_signerAd), _userBalanceBefore + _amount);
    assertEq(MAINNET_EURC.balanceOf(address(l1Adapter)), 0);
  }
...
contract Integration_PermissionedEurcFlows is IntegrationBase {
  ...
  /**
   * @notice Test `upgradeTo` EURC function on L2
   */
  function test_UpgradeTo() public {
    // Setup necessary data
    bytes memory _calldata = abi.encodeWithSignature('upgradeTo(address)', _newImplementation);

    // Use L2OpEURCBridgeAdapter owner to call `upgradeTo` function through the adapter
    vm.startPrank(_owner);

    // Call `upgradeTo` function
    l2Adapter.callEurcTransaction(_calldata);

    // Check that the EURC implementation has been upgraded
    assertEq(bridgedEURC.implementation(), _newImplementation);
  }

  /**
   * @notice Test `upgradeToAndCall` EURC function on L2
   */
  function test_UpgradeToAndCall() public {
    ...
    // Use L2OpEURCBridgeAdapter owner to call `upgradeToAndCall` function through the adapter
    vm.prank(_owner);
    ...
    // Check that the EURC implementation has been upgraded
    assertEq(bridgedEURC.implementation(), _newImplementation);
    assertEq(dummyImplementation(address(bridgedEURC)).minter(), _notOwner);
  }

  /**
   * @notice Test `updatePauser` EURC function on L2
   */
  function test_UpdatePauser() public {
    // Setup necessary data
    bytes memory _calldata = abi.encodeWithSignature('updatePauser(address)', _notOwner);

    // Use L2OpEURCBridgeAdapter owner to call `updatePauser` function through the adapter
    vm.startPrank(_owner);
    ...
    (, bytes memory _data) = address(bridgedEURC).call(abi.encodeWithSignature('pauser()'));

    //Get pauser from _data
    address _pauser = address(uint160(uint256(bytes32(_data))));

    // Check that the EURC pauser has been updated
    assertEq(_pauser, _notOwner);
  }

  /**
   * @notice Test `updateBlacklister` EURC function on L2
   */
  function test_UpdateBlacklister() public {
    // Setup necessary data
    bytes memory _calldata = abi.encodeWithSignature('updateBlacklister(address)', _notOwner);

    // Use L2OpEURCBridgeAdapter owner to call `updateBlacklister` function through the adapter
    vm.startPrank(_owner);
    ...
    (, bytes memory _data) = address(bridgedEURC).call(abi.encodeWithSignature('blacklister()'));

    //Get blacklister from _data
    address _blacklister = address(uint160(uint256(bytes32(_data))));

    // Check that the EURC blacklister has been updated
    assertEq(_blacklister, _notOwner);
  }

  /**
   * @notice Test `updateRescuer` EURC function on L2
   */
  function test_UpdateRescuer() public {
    // Setup necessary data
    bytes memory _calldata = abi.encodeWithSignature('updateRescuer(address)', _notOwner);

    // Use L2OpEURCBridgeAdapter owner to call `updateRescuer` function through the adapter
    vm.startPrank(_owner);
    ...
    (, bytes memory _data) = address(bridgedEURC).call(abi.encodeWithSignature('rescuer()'));

    //Get rescuer from _data
    address _rescuer = address(uint160(uint256(bytes32(_data))));

    // Check that the EURC rescuer has been updated
